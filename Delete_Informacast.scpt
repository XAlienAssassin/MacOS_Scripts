		...
		tell application "System Events"
			...
			repeat with i from (count items of dockApps) to 1 by -1
				try
					...
					-- Example from `defaults read com.apple.dock persistent-apps`:
					-- {
					-- GUID = 2333878649;
					-- "tile-data" =         {
					--    book = {length = 564, bytes = 0x626f6f6b 34020000 00000410 30000000 ... 04000000 00000000 };
					--   "bundle-identifier" = "com.singlewire.DesktopNotifier";
					--    "dock-extra" = 1;
					--    "file-data" =             {
					--        "_CFURLString" = "file:///Applications/DesktopNotifier.app/";
					--        "_CFURLStringType" = 15;
					--    };
					--    "file-label" = "InformaCast Desktop Notifier";
					--   "file-mod-date" = 132898539925823;
					--   "file-type" = 41;
					--   "parent-mod-date" = 202361044449663;
					-- };
				...
				on error
					-- continue search even if lookups in the property list fail (keys not found cause errors in script
					-- execution); the index record offset in the persistent-apps key list value will always be correct
				end try
			end repeat
		end tell
