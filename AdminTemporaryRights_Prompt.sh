...
# Prompt the user for the reason for admin access
reason=$(osascript -e 'text returned of (display dialog "Please provide a reason for needing temporary admin rights:" default answer "")')
echo "Reason for admin access: $reason"
...
# Set the run interval to run every 30 minutes
...
if [[ -f /private/var/userToRemove/user ]]; then
    ...
    log collect --last 30m --output "/private/var/userToRemove/${userToRemove}_$reason.logarchive"
fi
EOF
